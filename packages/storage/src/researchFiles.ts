...
import { applyFileTransaction, runSeriesFileTransaction, type FileTransactionOptions } from "./fileTransactions.js";
...
export async function listResearchSourceFiles(seriesRoot: string): Promise<ResearchSourceDocument[]> {
...
export async function createResearchSourceFile(
  ...
  const duplicate = (await listResearchSourceFiles(seriesRoot))
  ...
  return readResearchSourceFile(seriesRoot, parsed.id);
}
...
export async function updateResearchSourceFile(
  ...
  return runSeriesFileTransaction(seriesRoot, async (commit) => {
    const current = await readSourceAuthority(seriesRoot, researchSourcePath(seriesRoot, sourceId));
    ...
    const originalText = (await readResearchSourceFile(seriesRoot, sourceId)).originalText;
