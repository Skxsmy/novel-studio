...
Both limited execute routes claim the source tool message before the first authority write. The message persists `toolExecution` with request hash, `running` / `succeeded` / `failed` status, timestamps, optional result-message ID, and optional terminal error. Claims are serialized per Workshop session in the target single local server process; concurrent or later requests for the same tool message return `409` without repeating the authority mutation. Archived sessions cannot claim execution. A running execution blocks session archive/delete, and tool request messages cannot be removed through generic message deletion. These guards contain duplicate and lifecycle execution, but the composed Codex/detail/progression/result writes remain non-atomic until M5.6B.
