...
- Research Databases are library-level directories under `research-databases/<database-id>/`. Each directory owns `database.json`, SourceDocument version 3 authority, exact managed originals, versioned parsed content, Research Notes under `notes/`, transaction journals, and its own rebuildable identity-bound index. A database may link to several Series, but no Series owns or stores the database. Database selection, import, original-language search, Source management, and Research Note management work without an active Series. A Research Note stores bounded server-verified original-language Source evidence separately from author prose and does not become Series Canon without a Series-scoped Proposal.
