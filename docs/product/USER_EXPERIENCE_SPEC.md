...
- 当前知识库的关键词搜索位于阅读区上方，结果显示来源类型、命中词语言、命中通道和可读位置，并打开包含原始证据的精确结构块，将实际命中词滚动到当前视野并只高亮该词。结果标题与高亮词必须绑定作者已经提交的查询；作者随后修改但尚未提交的输入不能改写现有结果。只有包含命中位置的来源分页实际打开后才显示成功状态；分页失败时保留来源事实、属性和搜索结果，并显示能够重新请求该分页的真实重试动作，不能把分页读取失败误判为来源权威记录消失。还没有真实命令的 Delete、Archive、Reparse、批量操作、语义搜索和多知识库联合搜索必须缺席或明确不可用，不能以静态计数、假进度或示例结果代替。
