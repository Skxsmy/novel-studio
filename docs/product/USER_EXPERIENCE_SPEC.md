...
- `Research` 是左侧主导航中位于 `Review` 之后的独立工作区，Settings 仍固定在导航底部。进入页面不会离开当前 Series；刷新和重启按应用壳规则恢复最后打开的来源，来源不存在或损坏时回到资料列表并显示短错误状态。
- 页面采用稳定的三列资料管理布局，而不是卡片仪表盘：左列是可滚动来源列表和上传入口；中列是选中来源的标题、解析状态、原文结构/预览与后续检索内容；右列是属性表单。窄屏时来源列表成为可关闭抽屉，属性区在正文之后排列或成为侧栏，不能挤压原文到不可阅读。
...
- 还没有真实命令的 Delete、Archive、Reparse、批量操作、关键词搜索和语义搜索必须缺席或明确不可用，不能以静态计数、假进度或示例结果代替。
