...
当 UI 方向不清晰、用户明确表示“不好看 / 不统一 / 像后台表单 / 程序员视角太重”，或任务会显著改变主工作区布局时，不能只靠实现者凭感觉继续调 CSS。应先制作视觉预览：

1. 截取当前界面的真实截图，标出它的问题，例如信息层级错误、黑框过重、审计字段裸露、风格割裂。
2. 将当前截图作为反例参考，把本文件的视觉语言、中文文案规则、信息层级和具体页面目标一并交给图像模型。
3. 生成至少一版高保真 UI 预览，用来确认整体方向：布局、留白、边框、按钮、候选条、正文区域和侧边栏气质。
4. 人工查看预览，确认它是否真的比现状更接近“简洁、大气、美观、风格统一”的目标。
5. 将确认后的视觉方向拆成可实现项：设计令牌、组件样式、文案替换、布局调整和浏览器截图验收。
6. 若预览被采纳，应把预览图和可复现提示词保存到项目文档中，避免后续 AI 只根据聊天上下文猜测审美方向。
...
- 进入项目的图片必须使用稳定命名：`NS-###-页面-用途-vN.png`，例如 `NS-407-write-candidate-reference-v1.png`。
- 图片应集中保存在设计参考目录；若当前环境无法新建目录，可先记录原始生成路径和提示词，待有权限时再迁入。
