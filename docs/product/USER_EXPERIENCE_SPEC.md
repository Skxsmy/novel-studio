...
- 当前知识库的关键词搜索位于阅读区上方，结果显示来源类型、命中词语言、命中通道和可读位置，并打开包含原始证据的精确结构块，将实际命中词滚动到当前视野并只高亮该词。还没有真实命令的 Delete、Archive、Reparse、批量操作、语义搜索和多知识库联合搜索必须缺席或明确不可用，不能以静态计数、假进度或示例结果代替。
