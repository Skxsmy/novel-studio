...
- 原始资料、解析状态和权限属于当前获批的 Research 工作区。现有 `Notes` 页签来自未经要求的 NS-609 实现，现已冻结；它暂时保留只是为了避免在作者作出保留、重做或移除决定前破坏现有数据，不属于 NS-610 的设计范围。
- 搜索结果同时展示原文、来源位置和实际命中方式；NS-604 只显示已执行的关键词通道，不显示尚未执行的语义命中。
- 由 AI 生成的总结不能替代原文证据。
- 现有“加入项目”和“转入 Codex”控件属于冻结的 NS-609 兼容界面，不代表作者已经要求或接受这些工作流。NS-610 不得扩展、重做或依赖这些控件；是否保留或移除由作者之后单独决定。
...
- Source 行的键盘可达操作菜单提供 Archive；归档视图提供 Restore 和 Delete permanently。活动 Source 还提供 Replace file 或 Refresh web snapshot 以及 Reparse current original。替换和重新解析说明会保留旧版本。永久删除要求完整输入显示名称；如果冻结的 NS-609 数据引用该 Source，确认界面把会变成来源缺失的快照数量作为信息显示，并把仍依赖当前 Source 的待确认 Proposal 作为阻断原因。所有确认、冲突和暂时失败都保留当前行、草稿与阅读位置。窄屏使用同一操作和阻断信息，不把危险动作隐藏成无法解释的图标。
