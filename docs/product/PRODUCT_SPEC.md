...
- Agent tool calls are protocol messages, not assistant text. The assistant must not simulate `Tool Call` lines or claim a write occurred. For current Codex create/update intent, the server runner chooses a limited Codex tool workflow, buffers raw provider output, converts a structured draft through the Codex tool script, and creates a `role: tool` pending request. The frontend confirms that tool request before execution. Raw simulated tool-call text from the model must not stream into the UI or become the tool protocol.
- Each Agent author turn creates one durable Agent run containing ordered model, repair, tool-request, tool-result, and continuation steps. A normal prose response is a valid completed run and does not require a tool step. Every confirmed tool attempt returns its success or atomic failure result to the same run so the Agent continues without another author message; any later write request is a new pending tool message and requires its own author confirmation.
- Agent model steps use Provider structured output when the selected model profile declares that capability. A malformed structured result receives at most one server-recorded repair attempt. Profiles without structured output use a strict JSON text boundary that is visibly marked as degraded; invalid output after the repair attempt fails the run instead of being treated as prose or executable protocol.
- Agent runs and tool executions are never replayed automatically after interruption. On restart or session load, an unfinished run is reconciled to an explicit interrupted state after storage transaction recovery. The author may retry only a server-classified retryable step or abandon the run. Retry creates a new recorded attempt and preserves prior steps; abandon is terminal. Proposal conversion remains a separate explicit workflow and is not implied by retry or abandon.
...
- While a Workshop model call is in flight, the author must be able to stop it manually. Stopping aborts the current request/stream, preserves the already-created author message and bound attachments, and shows one stopped-status assistant message instead of duplicated error text.
...
- 分段记录来源、页码、章节或段落。
- 关键词 + 可选语义检索。
- 语义检索通过显式配置的 `EmbeddingModelProfile` 启用；默认推荐本地 `BAAI/bge-small-zh-v1.5` HTTP 服务，但不自动下载模型或静默改用云端服务。
- 结论可回指证据。
