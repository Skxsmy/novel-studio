...
- Codex Creation is a Workshop discussion/drafting mode for creating or modifying Codex material. It must load Codex-specific workflow and internal-interface guidance only for that mode, must not affect General Chat, and must not use the generic scene-content Proposal action until a dedicated Codex Proposal or approved Codex tool adapter exists. Its mode guidance must explain the boundaries between context selection, attachments, mention detection, detail Send to AI, scene-content Proposals, Review decisions, Codex entry writes, category/detail-type management, relations, and progressions so the AI cannot substitute one interface for another.
