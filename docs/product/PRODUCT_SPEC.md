...
- Research Database activation is a separate session-level selection, not a Context Basket item and not Series ownership. A Workshop session may activate several library-scoped Research Databases whether or not they are linked to the current Series; links only provide discovery and labeling. Activation exposes database metadata and server-owned read-only retrieval tools, never all Source text or all matched Chunks in the initial Provider context.
- When active Research Databases exist and the selected model supports native tool calls, both General Chat and Agent may iteratively call the server-owned Research tools to list permitted Sources, search bounded passages, and open exact or adjacent evidence. Read tools execute without a write confirmation, but the server enforces the selected-database allowlist, Source artificial-intelligence permission, per-call result limit, relevance threshold, deduplication, source diversity, cumulative context budget, call-count/no-progress limit, and original-language citations. Research write, import, property, and lifecycle commands are never model tools.
- A model without native tool calls may continue ordinary conversation, but it cannot be shown as capable of active database retrieval and must not receive a mechanically prefetched dump as a substitute. The interface must display that the selected model cannot use the active databases for this call.
- Research retrieval is an iterative tool loop, not automatic prompt stuffing. The first Provider request receives only the author-visible session selection, compact database capability metadata, and tool definitions. Tool results return only bounded passages with Research Database, Source, Chunk, hash, revision, language, location, and disclosed match channels. The model may refine the query or open neighboring evidence within the same author turn until it answers or exhausts the recorded budget.
...
首版的正式能力是导入、解析、检索和提炼用户提供的资料，而不是内置联网搜索。支持 TXT、Markdown、DOCX、文本型 PDF、EPUB、HTML，以及作者明确提交网页地址后的一次受控网页快照；扫描 PDF/OCR、Tavily、Firecrawl、自动联网研究、递归抓取和后台浏览器采集不属于首版。

`Research / 资料库` 是左侧主导航中的独立工作区，位于 Review 之后、Settings 之前；它不属于 Settings、Workshop 或 Codex 的内嵌页面。Research Database 是作品库级对象，不属于任何 Series；作者可以在没有打开 Series 时创建和管理多个互相隔离的知识库。每个知识库拥有独立来源、原件、解析结果和索引，并可以显式关联多个 Series。作者在这里选择知识库，导入 TXT、Markdown、DOCX、文本型 PDF、EPUB、HTML 或一次受控网页快照，查看有结构和位置的原文，检索当前知识库内的原语言内容，并管理显示名称、作者、声明语言、标签、人工智能使用权限和版权/使用备注。文件类型、原始文件名、大小、导入时间、解析器状态和来源定位等事实字段只读。25 MiB 是真实支持上限而不是只用于表单校验的数字；有效的大文本必须以有界分块、轻量导入响应和分页阅读方式完成，不能因为单段过长、段落很多或前端一次渲染全部内容而失败。尚未接入真实解析、检索或生命周期命令的格式和操作不得显示为可用控件。

Research Database 和 Source 必须有可理解的归档、恢复和永久删除路径。归档数据库或来源仍可由作者本地阅读，但只读并从检索、Workshop 激活和模型工具中排除。来源替换保留稳定 Source 身份和所有旧原件版本；重新解析不改写原件；网页刷新只在作者明确操作时建立新快照。永久删除必须在服务器检查当前 revision、完整名称确认、Workshop 和待确认 Proposal 等外部引用，显示具体阻断原因，并严格限制在目标数据库根内。已有的冻结 NS-609 数据只能作为防止破坏现存文件的只读兼容检查，不能由此推断 Research Note 生命周期是正式需求。归档不是无限保留的替代品，也不能用级联删除静默破坏外部历史。

资料库必须做到：

- 保留原始文件和哈希。
- 默认只检索当前知识库；明确多选时可以联合返回带知识库标识的结果，但知识库的权威文件、索引、权限和生命周期保持隔离。
- 分段记录来源、页码、章节或段落。
- 关键词 + 可选语义检索。
...
- 结论可回指证据。
- 参考资料与故事 Canon 严格分离；现存但被冻结的 NS-609 Research Note 实现不构成已批准的产品范围。
