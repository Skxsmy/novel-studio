...
数据库、来源和 Research Note 必须有可理解的归档、恢复和永久删除路径。归档数据库或来源仍可由作者本地阅读，但只读并从检索、Workshop 激活和模型工具中排除。来源替换保留稳定 Source 身份和所有旧原件版本；重新解析不改写原件；网页刷新只在作者明确操作时建立新快照。永久删除必须在服务器检查当前 revision、完整名称确认、Workshop、Research Note 和待确认 Proposal 引用，显示具体阻断原因，并严格限制在目标数据库根内。归档不是无限保留的替代品，也不能用级联删除静默破坏外部历史。
