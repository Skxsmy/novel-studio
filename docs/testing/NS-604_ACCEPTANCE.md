...
| NS-604-A12 | passed | Web/browser tests | Research component/app-shell tests, bounded reader paging, recoverable page failures, submitted-query identity, exact-match highlighting, and `tests/e2e/ns-604-research-formats.spec.ts` | Web paging/navigation/failure suite passes 9 tests; real Chrome injects and counts one 500 page failure, preserves the Source and retryable result without false success, then opens and highlights the submitted Japanese term despite edited input. Both structured-format and 3 MiB workflows pass, and connected diagnostics independently place the final large-file term in the viewport |
