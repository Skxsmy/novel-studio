...
| NS-604-A12 | passed | Web/browser tests | Research component/app-shell tests, bounded reader paging, recoverable page failures, submitted-query identity, exact-match highlighting, and `tests/e2e/ns-604-research-formats.spec.ts` | Web paging/navigation/failure suite passes 9 tests; real Chrome injects and counts one 500 page failure, preserves the Source and retryable result without false success, then opens and highlights the submitted Japanese term despite edited input. Both structured-format and 3 MiB workflows pass; the committed connected preview independently preserves the submitted large-file result while the input contains a different unsubmitted query and places its exact term in the viewport |
| NS-604-A13 | passed | commands/probes | focused/full checks, generated real files, API/disk/network/25 MiB stress probes, browser diagnostics, and repository ledger | Latest `npm.cmd run check` passes 186-file documentation, all workspace typechecks, 59 files/586 tests, and production build; focused Chrome, final 25 MiB, staged diff, and post-implementation status-scope checks pass |
