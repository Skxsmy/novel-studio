...
| 用例 ID | 目标状态 | 预期行为 |
|---|---|---|
| BA-M4-001 | 待实现 | 用户可配置模型 Provider，连接测试显示模型能力、错误分类和是否可流式输出。 |
| BA-M4-002 | 待实现 | 场景页可打开上下文预览，看到将发送的场景目标、正文选区、相邻摘要、设定状态和钉选资料。 |
| BA-M4-003 | 待实现 | 被标记为 `never` 或未来剧情的资料不会出现在上下文预览和实际调用记录中。 |
| BA-M4-004 | 待实现 | 发起一次非写入型 AI 调用后，调用日志保存模型、角色、提示词版本、上下文摘要和用量。 |
| BA-M4-005 | 待实现 | 云端禁用时不会发生云端请求，也不会从本地模型静默回退到云端模型。 |
