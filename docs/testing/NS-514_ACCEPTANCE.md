...
| NS-514-A05 | passed | automated test | `apps/web/src/app/ReferenceAppShell.test.tsx` — `renders only the manifest shell regions in reference order`; `uses Overview as the initial workspace and preserves disabled navigation` | Passed; the reference appbar, workspace order, A39-approved initial Overview state, disabled Review/Research controls, project switch, dialog order, tokens, and compact selectors are present without legacy shell regions. The binding's original Codex default remains historical P1 source evidence rather than the current product default |
...
- Workshop P6 implementation begins at `ee1a4f3`; browser-discovered repair
  ...
  interface-to-Context-Bundle browser evidence. Implementation commit
  `ae7452b` isolates
