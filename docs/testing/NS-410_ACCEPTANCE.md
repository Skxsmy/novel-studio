...
| NS-410-A09 | passed | Slice 5 storage and server tests verify same-scene block position changes effective Codex fields before/after write-block progressions. The audit repair extends same-scene block-position isolation to world and relationship Progression effective state. The post-Slice 11 audit repair documents and tests `effectiveToSceneId` as an inclusive end-scene boundary with no block-level end boundary in v1. |
...
| Post-Slice 11 Audit Repair | Rechecked `NS-410_SLICE_1_11_AUDIT.md` findings S1-S5 against source; fixed confirmed code/spec gaps without changing UI behavior. | A04, A05, A06, A09, A14 |
...
### Slice 8: Write Embedded Progression Blocks And Scene-Level Commands
...
- Read paths: Write block document load, Codex entry/detail type/progression list reads, and effective-entry projection used by storage/API/Codex regressions.
- Write paths: scene document saves, dedicated scene progression-block create/delete commands, and Progression update saves.
- Derived paths: plain text/count/mention projection still derives from blocks; effective-entry projection remains covered outside the Write author preview surface.
- User-visible entries: embedded Write progression block controls.
- Delete path: deleting from the embedded block removes the block and linked Progression record together, or returns blocker reasons while leaving both records intact.
...
- Write can create an embedded Codex progression block, edit its target entry/field/operation/summary/body, collapse/expand UI state without writing it to authority, autosave the linked Progression, and delete the block through the editor.
- Codex progression copy is centralized in `uiText.writeProgression`; shared action labels such as Delete use `uiText.actions`.
- Saved scene document bodies contain `codexProgression` blocks and do not persist UI-only chrome.
...
- Web tests cover the embedded progression flow without requiring a duplicated side panel.
...
- High: deleting a Codex progression block no longer drops unsaved scene prose. Write now saves a dirty `SceneBlockDocument` draft first, uses the saved revision for the delete command, and only then accepts the server-returned scene.
- High: Codex progression insertion and deletion are no longer frontend/storage best-effort multi-file sequences. Storage exposes `createSceneProgressionBlock` and `deleteSceneProgressionBlock` scene-level commands that write the scene JSON and Progression JSON in one recoverable `applyFileTransaction`.
- Medium: first-block Codex progression preview now reads effective entry state at the previous narrative scene instead of falling back to raw Codex baseline.
- Low: the Slice 8 copy claim is narrowed to Codex-progression-specific `uiText.writeProgression` keys, with shared actions such as Delete coming from `uiText.actions`.

Logic closure boundary verified:

- Authority source: scene `SceneBlockDocument` JSON and unified `codex/progressions/<id>.json`.
- Read paths: scene document load, current-scene progression list for embedded block hydration, and effective-entry projection at previous block or previous scene.
- Write paths: dirty draft scene save, scene-level progression-block create/delete, and linked Progression update.
- Derived paths: scene plain text/count/mentions still derive from block projection; effective-state preview behavior is not persisted as authority.
- User-visible entries: embedded Codex progression block controls.
...
- Write paths: no new authority write path was added in Slice 9; Codex baseline edits still use the existing entry update path, and Write-sourced Progression edits remain in the Write Codex progression block workflow.
...
- Removed normal editor Save buttons and normal save-status noise from Write/Codex editing surfaces. Write scene edits, inline Codex progression edits, and Codex entry edits autosave; save failure/conflict messaging remains visible.
...
### Post-Slice 11: Audit Finding Repair

Status: command-verified on 2026-06-30; user visual acceptance remains separate.

Audit file reviewed:

- `docs/testing/NS-410_SLICE_1_11_AUDIT.md` was retained in the workspace and not committed, per user instruction.

Finding review:

| Finding | Source review | Resolution |
| --- | --- | --- |
| S1 generic Progression CRUD can bypass scene-level write-block synchronization | Confirmed. Generic create/update accepted `write-block` sources that only proved a block ID existed. | Storage now requires a `write-block` source to reference a `codexProgression` scene block whose `progressionId` points back to the same Progression. New write-block Progressions must come through `createSceneProgressionBlock`; storage/server tests now use the real scene command for write-block fixtures. |
| S2 generic Progression hard delete has weaker rollback semantics | Confirmed for `deleteCodexProgression`. | Generic Progression hard delete now uses `applyFileTransaction` with a single delete mutation after blocker checks, matching transaction-backed recovery behavior for the authority file. |
| S3 atomic writes lack explicit flush | Confirmed. Same-directory temp write and rename existed, but file/directory sync was not explicit. | `atomicWrite` and transaction temp writes now sync file handles before rename and best-effort sync touched directories after replacement/transaction renames where Node/Windows permits. |
| S4 hardcoded UI copy remains in touched Write/editor surfaces | Confirmed for NS-410-touched Write/editor/session examples. | Moved touched Write/editor/session visible/default copy into `uiText` without changing displayed strings, layout, controls, or UI behavior. Wider Plan/Overview hardcoded copy remains a separate UI/i18n debt and is not claimed closed in this no-UI-change repair. |
| S5 `effectiveToSceneId` lacks documented NS-410 semantics | Confirmed as a specification gap, not a runtime failure. | Kept the existing contract/API field to preserve compatibility; documented and tested it as an inclusive end-scene boundary that must be later than `effectiveFromSceneId`, with no block-level end boundary in v1. |

Commands run before final full regression:

| Command | Result |
| --- | --- |
| `npm.cmd run test -w @novel-studio/storage -- repository.test.ts` | Initial run exposed a test fixture mistake in the rewritten block-position test; after fixing, rerun passed with 57/57 tests. |
| `npm.cmd run test -w @novel-studio/server -- app.test.ts context-routes.test.ts` | Initial run exposed stale test fixture revision/variable references; after fixing, rerun passed with 13/13 tests. |
| `npm.cmd run test -w @novel-studio/web -- AppShell.test.tsx EditorSurface.test.tsx` | Passed with 54/54 tests. |
| `npm.cmd run test -w @novel-studio/storage -- repository.test.ts json-authority.test.ts` | Passed with 63/63 tests. |
| `npm.cmd run build` | Passed; Vite produced split chunks and no large-chunk warning. Largest emitted chunks remained `editor-codemirror` at 494.48 kB and `editor-tiptap` at 439.56 kB. |
| `npm.cmd run test` | Passed with server 25/25, web 59/59, AI 20/20, and storage 68/68 tests. |
| `git diff --check` | Passed with line-ending warnings only. |
