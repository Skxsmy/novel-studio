...
- Active task: `NS-604 / M6.3 Durable Research Source And Original-language Lexical Index`; A01-A13 pass after the 3 MiB defect closure and exact-match navigation correction, while the latest correction commit and A14 author visual acceptance remain pending.
...
- Next mainline action: commit the exact-match navigation correction, then present the connected large-source Research page for the author's A14 decision and keep NS-604 in progress until that decision is recorded. Do not begin hybrid retrieval, model tools, or Workshop integration inside NS-604.
...
- Post-`f6307e8` connected inspection found that a search result opened the correct 40-Block page but left the exact term outside the viewport when the matched Block was large. The task worktree now renders a term-level marker, scrolls it after the page commits, cancels stale navigation on source changes, and verifies the marker is in the real Chrome viewport without coloring the full Block.
