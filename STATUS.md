...
- Current active line: `M5` Workshop, Proposal, and Review. `M5.1` through `M5.4` are command/function verified as of 2026-07-01: Proposal v2, Proposal/Review JSON storage and APIs, Review UI on real Proposal data, and Workshop sessions/context/single-role calls are implemented. User visual acceptance is still pending, and agent-owned screenshot acceptance is prohibited. The next slice is `M5.5` Workshop Message To Proposal Deep Link.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage by user decision on 2026-06-30. M5.1-M5.4 are command/function verified, but M5 is not complete: Workshop-to-Proposal deep links, Tool Plans/Grants, Council, final responsive/state sweep, and user visual acceptance remain unfinished.
...
- M5.1-M5.4 are command/function verified. Proposal v2 extends the existing Proposal contract, Proposal/Review storage and routes exist, Review uses real Proposal APIs, and Workshop persists sessions/messages/context baskets and can run single-role calls through existing context/model-call paths. M5.5 must add Workshop message to Proposal deep links and source-message status sync before Workshop Proposal cards are claimed. User visual acceptance remains pending.
...
- Review and Workshop are implemented only through the M5.4 scope. Review decision flows and Workshop session/context/single-role calls are command/function verified, but Workshop-generated Proposal cards, exact source-message deep links, Tool Plans, Grants, Council, and final user visual acceptance remain incomplete.
...
- Review and Workshop are not complete product workflows beyond the M5.4 scope. Proposal decisions and Workshop sessions/context/single-role calls exist, but Workshop-to-Proposal deep links, source-message status sync, Tool Plans/Grants, Council, and final user visual acceptance remain pending.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage. Continue with M5 through `docs/tasks/M5.md` and `docs/testing/M5_ACCEPTANCE.md`; the next slice is M5.5 Workshop Message To Proposal Deep Link. Figma checklist review and user visual acceptance remain separate from command/function checks; agents must not run screenshot-based UI acceptance.
