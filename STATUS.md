...
- Current active line: `M5` Workshop, Proposal, and Review. `M5.1` through `M5.5` are command/function verified as of 2026-07-01, with post-M5.5 audit repair also command-verified on 2026-07-01: Proposal v2, Proposal/Review JSON storage and APIs, Review UI on real Proposal data, Workshop sessions/context/single-role calls, and Workshop-message-to-Proposal deep links are implemented. User visual acceptance is still pending, and agent-owned screenshot acceptance is prohibited. The next slice is `M5.6` Tool Plan, Grant, And Command Adapter.
...
- Review and Workshop are implemented only through the M5.5 scope plus follow-up UI/model-selection repair. Review decision flows, read-only non-pending details, Workshop session/context/model-selected single-role calls, Workshop-generated Proposal cards, exact source-message deep links, and status sync are command/function verified, but Tool Plans, Grants, Council, and final user visual acceptance remain incomplete.
...
- Review and Workshop are not complete product workflows beyond the M5.5 scope. Proposal decisions, Workshop sessions/context/single-role calls, Workshop-to-Proposal deep links, and source-message status sync exist, but Tool Plans/Grants, Council, and final user visual acceptance remain pending.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage. Continue with M5 through `docs/tasks/M5.md` and `docs/testing/M5_ACCEPTANCE.md`; the next slice is M5.6 Tool Plan, Grant, And Command Adapter. Figma checklist review and user visual acceptance remain separate from command/function checks; agents must not run screenshot-based UI acceptance.
