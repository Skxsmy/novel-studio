...
- Review and Workshop are implemented only through the M5.5 scope plus follow-up UI/model-selection/context-selector repair. Review decision flows, read-only non-pending details, Workshop session/context/model-selected single-role calls, Workshop-generated Proposal cards, exact source-message deep links, context menu scope selection, auto-linked Codex materialization, and status sync are command/function verified, but Tool Plans, Grants, Council, and final user visual acceptance remain incomplete.
...
- Review and Workshop are not complete product workflows beyond the M5.5 scope plus the 2026-07-02 context-selector and General Chat repairs. Proposal decisions, Workshop sessions/context/single-role calls, compact context selection, default General Chat with custom system prompt and no Proposal entry, Workshop-to-Proposal deep links for non-General replies, and source-message status sync exist, but Tool Plans/Grants, Council, and final user visual acceptance remain pending.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage. Continue with M5 through `docs/tasks/M5.md` and `docs/testing/M5_ACCEPTANCE.md`; the next slice is M5.6 Tool Plan, Grant, And Command Adapter. The 2026-07-02 Workshop context-selector and General Chat repairs are command/function verified but not visual acceptance. Figma checklist review and user visual acceptance remain separate from command/function checks; agents must not run screenshot-based UI acceptance.
