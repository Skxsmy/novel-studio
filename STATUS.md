...
- Current active line: `M5` Workshop, Proposal, and Review. `M5.1` through `M5.5` are command/function verified as of 2026-07-01, with post-M5.5 audit repair also command-verified on 2026-07-01. The 2026-07-02 Workshop context-selector repair and Codex detail `Send to AI` audit are command/function verified: Workshop no longer exposes a visible right-side Context Basket or Preview Context button, the compact composer menu backs the same persisted context data, story-scope selections can auto-link policy-allowed Codex entries, and the Codex detail `Send to AI` switch is verified to filter actual ContextBundle content. User visual acceptance is still pending, and agent-owned screenshot acceptance is prohibited. The next slice is `M5.6` Tool Plan, Grant, And Command Adapter.
...
- Review and Workshop are not complete product workflows beyond the M5.5 scope plus the 2026-07-02 context-selector repair. Proposal decisions, Workshop sessions/context/single-role calls, compact context selection, Workshop-to-Proposal deep links, and source-message status sync exist, but Tool Plans/Grants, Council, and final user visual acceptance remain pending.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage. Continue with M5 through `docs/tasks/M5.md` and `docs/testing/M5_ACCEPTANCE.md`; the next slice is M5.6 Tool Plan, Grant, And Command Adapter. The 2026-07-02 Workshop context-selector repair is command/function verified but not visual acceptance. Figma checklist review and user visual acceptance remain separate from command/function checks; agents must not run screenshot-based UI acceptance.
