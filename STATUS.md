...
- Review and Workshop are not complete product workflows beyond the M5.5 scope plus the 2026-07-02 context-selector repair and the 2026-07-03 General Chat/settings/message-attachment/context-delivery/UI-interaction repairs. Proposal decisions, Workshop sessions/context/single-role calls, compact context selection, default General Chat with fully visible custom system prompt and no Proposal entry, optional streaming, visible collapsible reasoning display, parsed user-message attachments, attachment-only sends, manual Stop for in-flight calls, same-session chat-history context, message deletion for unlinked chat messages, Workshop-to-Proposal deep links for non-General replies, and source-message status sync exist, but Tool Plans/Grants, Council, and final user visual acceptance remain pending.
