...
- Current active line: `M5` Workshop, Proposal, and Review. Formal M5 startup began on 2026-07-01 at `M5.0` Current Feature Protection Lock. This slice is tests/docs only unless protection tests expose a current breakage; Workshop/Review UI, Proposal v2 contracts, and Tool Plan execution are not started.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage by user decision on 2026-06-30. M5 is formally active at M5.0 Current Feature Protection Lock, but no Workshop/Review UI, Proposal v2 contract, Proposal storage/API, or Tool Plan execution has been implemented.
...
- M5.0 is active. M5 Workshop/Proposal/Review design and implementation are not accepted yet, and the M5.0 protection lock must map and verify existing Write/Codex/Settings/AI/context/JSON-authority behavior before later M5 slices change contracts or UI.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage. Continue with M5 through `docs/tasks/M5.md` and `docs/testing/M5_ACCEPTANCE.md`; the current slice is M5.0 Current Feature Protection Lock. Figma/user visual acceptance for later M5 UI remains separate from command/browser checks.
