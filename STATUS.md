...
- Active task: `NS-604 / M6.3 Durable Research Source And Original-language Lexical Index`; A01-A13 pass in the task worktree after the reported 3 MiB defect closure, while A14 author visual acceptance and task-scoped commits remain pending.
...
- Next mainline task: `NS-604 / M6.3 Durable Research Source And Original-language Lexical Index`; it remains active until final repository evidence is committed and the separate A14 decision is recorded. The first unstarted successor is NS-605, followed independently by NS-606, NS-607, and NS-608.
- Next mainline action: commit the NS-604 implementation and evidence, then present the connected large-source Research page for the author's A14 decision. Do not begin hybrid retrieval, model tools, or Workshop integration inside NS-604.
...
- NS-604 planning commit `33a8e0f` records ADR-0020 and A01-A14 before runtime edits. Implementation commit `1017c88` adds SourceDocument version 3, TXT/Markdown/DOCX/PDF/EPUB/HTML/web parsing, exact binary/content authority, explicit v2 migration, one identity-bound index per Research Database, and single-database original-language lexical search. The later 3 MiB defect closure replaces quadratic/unbounded plain-text parsing, bounds Blocks, returns lightweight Source views, pages the reader, and opens search results on the matched Block page. A direct 25 MiB HTTP probe and connected 3 MiB browser workflow pass, but NS-604 remains in progress until final evidence is committed and A14 is decided. It does not provide hybrid cross-language retrieval, model-facing tools, or Workshop integration; those are separately planned in NS-605 through NS-608 under ADR-0021.
...
- Current implementation capability stops at local single-database lexical retrieval. The `model-context` purpose flag only filters `never` Sources; no Workshop or Provider path calls it. Hybrid cross-language retrieval, read-only model tools, session activation, bounded iterative retrieval, and real saved-key author workflows are all unfinished and must not be presented as implemented.
