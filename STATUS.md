...
- NS-410 completed JSON authority paths currently cover scene manuscripts, unified Progression records, and character knowledge. Markdown/Word are boundary formats for those completed paths; SQLite/FTS5 remains rebuildable derived state. Remaining YAML/Markdown authority paths are inventoried in `docs/testing/NS-410_ACCEPTANCE.md`, remain open NS-410 work for the later JSON authority completion slice, and must not be described as migrated. Slice 7 gives Write a native `SceneBlockDocument` editor for ordinary scene blocks; repaired Slice 8 adds embedded Write progression blocks plus the current-scene progression panel with scene-level atomic create/delete commands, dirty-draft-safe deletion, and previous-scene first-block previews. The user-requested project lifecycle repair adds Library Trash, Restore, and permanent project-directory delete after exact project-name confirmation; it is Project Recovery lifecycle evidence, not an NS-410 JSON-authority completion claim. The Write scene-content path no longer uses CodeMirror/`EditorSurface`, while Codex editing surfaces may still use `EditorSurface`.
...
11. Slice 9 Codex baseline/history/effective-at-scene UI. Next implementation slice.
...
Continue NS-410 with Slice 9 from `docs/tasks/NS-410.md`: Codex baseline/history/effective-at-scene UI. Slices 1-8 plus Slice 7A audit repair are verified and recorded in `docs/testing/NS-410_ACCEPTANCE.md`; Slice 9 should distinguish baseline from scene-effective fields and list Progression history without fake future data or exposing engineering-only IDs in the main author workflow.
