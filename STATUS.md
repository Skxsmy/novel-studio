...
- Next mainline action: A04 through A08 real-author behavior, adversarial boundaries, and recovery are passed after live defects or stale tests were repaired; run A09 declared 25 MiB exact-ingestion/search and 4,096-Chunk vector timing, memory, and disk probes next.
...
- NS-608 planning commit `d520b21` freezes the saved-key `deepseek-v4-pro` map, and preflight commit `8b2f10a` proves host credential resolution, native confirmation, pointer confinement, cleanup, and byte-exact real-library safety. A04 passes 3/3 independent four-turn cross-language sessions. A05 passes 3/3 independent eight-turn natural-correction sessions after hardening natural write intent, canonical final-value prompts, semantic same-target replay suppression, confirmation continuation, and deterministic authority grading. A06 passes both three-trial five-turn batches: ordinary author work remains tool-free until evidence is explicitly needed, and conflicting Japanese/English sources retain separate facts, uncertainty, and false-friend boundaries. A07 passes 3/3 four-turn prompt-injection/permission sessions plus strict gateway, policy, citation, route, and cross-target adversarial regressions. A08 passes 3/3 real post-adapter-start cancellations plus deterministic Provider failure, retry, restart, stale citation, deactivation, and no-replay recovery coverage. A09-A13 remain open.
