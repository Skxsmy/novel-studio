...
- Next mainline action: A04 cross-language continuity is 3/3 passed after live defects were repaired; implement and run A05 natural-correction/Codex multi-session trials, then continue balanced-triggering, adversarial, recovery, performance, connected UI, and final audit closure.
...
- NS-608 planning commit `d520b21` freezes the saved-key `deepseek-v4-pro` map, and preflight commit `8b2f10a` proves host credential resolution, native confirmation, pointer confinement, cleanup, and byte-exact real-library safety. A04 now passes 3/3 independent four-turn cross-language author sessions after correcting citation grading, failed-suite exit status, DeepSeek thinking-mode Research finalization, bounded partial-word lexical ranking, and CJK multi-term recall; A05-A13 remain open.
