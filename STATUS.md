...
- M4：进行中。`NS-401` 至 `NS-405` 已完成；下一任务是 `NS-406`。
...
- NS-404/NS-405 只提供最小设置页和上下文预览入口；完整上下文分组 UI、调用后快照查看和浏览器自动验收属于 `NS-409`。
...
说明：下一步应把当前上下文预览中的“编辑职责占位”替换为可审计的角色与提示词模板文件，并确保每次后续调用都能记录 PromptTemplate ID 和版本。
