...
6. Codex Details follow-up: command-verified on 2026-06-26. Tags are removed from the Codex entry path; reusable detail types are centrally managed per category; detail values reuse `EditorSurface`. A Playwright screenshot self-check passed after the in-app browser was unavailable, but this is not user visual acceptance.
