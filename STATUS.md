...
- Current active line: `M5` Workshop, Proposal, and Review. `M5.1` through `M5.5` are command/function verified as of 2026-07-01: Proposal v2, Proposal/Review JSON storage and APIs, Review UI on real Proposal data, Workshop sessions/context/single-role calls, and Workshop-message-to-Proposal deep links are implemented. User visual acceptance is still pending, and agent-owned screenshot acceptance is prohibited. The next slice is `M5.6` Tool Plan, Grant, And Command Adapter.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage by user decision on 2026-06-30. M5.1-M5.5 are command/function verified, but M5 is not complete: Tool Plans/Grants, Council, final responsive/state sweep, and user visual acceptance remain unfinished.
...
- M5.1-M5.5 are command/function verified. Proposal v2 extends the existing Proposal contract, Proposal/Review storage and routes exist, Review uses real Proposal APIs, Workshop persists sessions/messages/context baskets and can run single-role calls through existing context/model-call paths, and Workshop messages can create durable Proposal cards with exact Review/source-message deep links. User visual acceptance remains pending.
