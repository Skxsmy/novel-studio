...
- Current active line: `M5` Workshop, Proposal, and Review. `M5.0` Current Feature Protection Lock passed on 2026-07-01 after adding targeted guard tests and running focused/full verification. The next slice is `M5.1` Proposal v2 Contract And State Machine; Workshop/Review UI and Tool Plan execution are not started.
...
NS-410 is command-verified through post-Slice 11 audit repair. Project Recovery is accepted for the current stage by user decision on 2026-06-30. M5.0 is command-verified, but no Workshop/Review UI, Proposal v2 contract, Proposal storage/API, or Tool Plan execution has been implemented.
...
- M5.0 is passed. M5 Workshop/Proposal/Review design and implementation are not accepted yet; M5.1 must start with the existing Proposal contract rather than a parallel model.
