...
- Before asking the user to decide, confirm, or compare product behavior, explain the decision in concrete natural language. Name the exact page or panel, the visible control or content, the triggering action, the data or state that would change, and the nearby behavior that would remain unchanged.
- Do not compress a decision into shorthand that can refer to multiple UI or data concepts. Distinguish, for example, a page from a popover inside that page, a source selector from the content it filters, and a displayed effective value from the rule used to build a preview. When modes, story positions, or lifecycle states matter, include a concrete before/after example.
- When explaining a decision to the user, do not abbreviate words, phrases, feature names, UI locations, states, actions, conditions, consequences, or causal relationships. Write the complete intended meaning every time, even when it is repetitive. Do not substitute acronyms, internal identifiers, shortened English labels, slash-separated fragments, or translation-like noun strings for a complete natural-language explanation. If the product's visible label is itself short, quote that label and immediately explain in a complete sentence exactly what it controls and what it does not control.
- Present recommendations as recommendations, state their reason and tradeoff, and keep them separate from already approved requirements. A recommendation, inference, code observation, or proposed default is never a user decision until the user explicitly accepts it.
- Before raising a product question, check the authoritative specification and recorded user decisions. Do not reopen an already decided behavior under new wording. If a genuinely new edge case is not settled by the existing decision, explain precisely why the recorded rule does not answer it.
- After the user decides, restate the result with the same concrete objects and boundaries before recording it. If the user corrects an interpretation, remove or supersede every contradictory active statement instead of leaving ambiguous alternatives in live task or product documents.
