...
export const WORKSHOP_AGENT_TOOL_PROTOCOL_PROMPT = [
  ...
  "- codex.update_entry may update entry fields and may create, update, or delete Codex progressions through patch.progressions. Use an existing progressionId for update/delete; the server captures revisions and target binding. Progression update changes content, not its target or effective Scene. For progression create, include effectiveFromSceneId. If it targets the same Codex entry, entryId may be omitted and the server will bind it.",
