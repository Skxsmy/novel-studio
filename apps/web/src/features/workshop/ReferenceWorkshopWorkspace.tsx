  ...
  function renderPlaceholder() {
    ...
    function renderContextPopover() {
      ...
      if (contextList) return (
        <section aria-label={text.labels.chooseContext} className="wr5-popover wr5-context-popover" data-wr5-floating role="dialog">
          {renderContextList()}
        </section>
      );
      return (
    ...
    return <>
      ...
      <main className="wr5-conversation">
        ...
        </div>{renderContextPopover()}{modelMenuOpen ? <section aria-label={text.labels.chooseModel} className="wr5-popover wr5-agent-popover" data-wr5-floating role="dialog"><div className="wr5-model-list">{activeModelProfiles.map((profile) => <label className="wr5-model" key={profile.id}><input checked={selectedModelProfile?.id === profile.id} name="wr5-model" onChange={() => void selectModel(profile.id)} type="radio" /><strong>{profile.model}</strong></label>)}</div></section> : null}{runtimeOptionsOpen ? <section aria-label={text.labels.runtimeOptions} className="wr5-popover wr5-agent-popover wr5-runtime-popover" data-wr5-floating role="dialog"><div className="wr5-agent-options"><label className="wr5-agent-option"><input checked={useStreamingResponses} onChange={(event) => setUseStreamingResponses(event.currentTarget.checked)} type="checkbox" /> {text.labels.streamResponsesFull}</label>{renderReasoningControls()}</div></section> : null}</div></footer>
      </main>
    </>;
  }
