  ...
  return (
    ...
    >
      <div className="scene-card-top">
        ...
        <span className="draft-dot">{scene.status}</span>
      </div>
      <h4>{scene.title}</h4>
      <p>{scene.summary || "尚未填写规划摘要。"}</p>
      <div className="planning-card-meta">
        <span>{scene.pov || "未设 POV"}</span>
      ...
      {scene.planningState !== "aligned" && <span className="divergence-chip">{scene.planningState}</span>}
    </article>
  );
}
  ...
  return (
    <section className="content-page planning-page" data-testid="planning-page">
      ...
      <div className="planning-toolbar">
        ...
        {filter && <button className="filter-chip" onClick={() => setFilter(null)}>筛选：{filter.value} ×</button>}
        <span className="planning-revision">数据 {board.revision.slice(0, 8)}</span>
      </div>
      ...
      <div className="planning-layout">
        <div className="planning-canvas" aria-busy={busy}>
          ...
          {mode === "timeline" && <div className="timeline-view" data-testid="timeline-view">
            <section><h3>叙事时间线<small>读者看到的顺序</small></h3>{timelines.narrative.map((scene) => <button className={selectedSceneId === scene.id ? "selected" : ""} onClick={() => setSelectedSceneId(scene.id)} key={scene.id}><span>{scene.narrativeIndex}</span><div><strong>{scene.title}</strong><small>{scene.pov || "未设 POV"}</small></div></button>)}</section>
