...
- Done in current M5 code slice: Workshop Codex Creation is a selectable discussion/drafting mode for Codex entry/category/detail/relation/progression work. It uses the researcher/research call path, dynamically loads Codex workflow/internal-interface guidance only for that mode, can stream replies, permits deletion of unlinked chat messages, and is barred in UI plus storage/API from the generic scene-content Proposal creation path until a dedicated Codex Proposal or approved Codex tool adapter exists.
