...
- Open Slice D2: Editor Foundation. Replace the ad hoc contentEditable approach with a dedicated editor slice. CodeMirror 6 is the preferred implementation direction unless a concrete spike blocker is recorded; ProseMirror/Tiptap and Lexical are backup options for richer schemas. The slice must cover document/selection/transaction state, decoration-based Codex marks, Chinese IME safety, duplicate-input prevention, paste cleanup, undo/redo, paragraph indentation rules, scroll-aware anchored popovers, and writer-focused status/word-count behavior. Do not keep adding unrelated one-off key handlers indefinitely.
