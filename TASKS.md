...
- Done in current M5 UI repair: Workshop's left session sidebar follows `docs/design/ui-redesign/M5_WORKSHOP_UI_REDESIGN_REVIEW.html` for the compact thread-list style. Add remains a session-kind menu, session actions now collect archive/restore, branch, permanent delete, deferred import, and export options in the sidebar header, and rows show Chat/Agent/Archived pills without exposing audit fields. Command/function verification passed; user visual acceptance remains separate.
- Done in current M5 UI repair: the app-level project sidebar's collapsed state now keeps the light color system but uses the redesign's narrow rail structure: project-initials brand block, icon plus short-label workspace buttons, and icon-only Switch Project/Settings footer actions. The expanded sidebar is intentionally unchanged.
