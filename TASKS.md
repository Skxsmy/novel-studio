...
- Done in current Codex UI follow-up: the detail-open Codex workspace now makes the category rail and Entry Index auxiliary-width panels while the selected entry editor takes the main workspace, with a compressed entry header so editing controls appear earlier.
...
- [!] `PROJECT-RECOVERY` Project-level recovery roadmap.
  ...
  - Codex Details follow-up: command-verified on 2026-06-26; screenshot self-check for the modal follow-up was skipped by user instruction. A same-day UI follow-up widened the detail-open workspace and fixed Canon preview close/bounds behavior. This is not user visual acceptance.
