...
- Done in current NS-410 Write editor refactor follow-up: Write now uses a continuous Tiptap-backed manuscript editor instead of the visible repeated block-card/textarea UI. `SceneBlockDocument` remains the saved authority, paragraph add/delete and Focus remain available, the floating format panel/destructive text-type selector was removed, legacy heading/scene-break data still round-trips, Codex progression creation/edit/delete/collapse now happens through a resizable inline manuscript component while Scene Brief lists progressions as a read-only overview, Write/Codex progression/Codex entry editing autosaves without normal Save buttons, and automated command checks pass. User visual acceptance remains separate.
...
- [-] `NS-410` Block Write Editor and Unified Codex Progression.
  ...
  - Current slice state: Slice 1 passed for contracts and JSON file foundation; Slice 2 passed for scene JSON authority with legacy `content` API compatibility; Slice 3 passed for scene document API and Markdown export; Slice 4 passed for unified Progression JSON storage and CRUD API; Slice 5 passed for projection engine and effective entry API; Slice 6 passed for Context Builder and preview projection; Slice 7 passed for the native Write ordinary block editor; Slice 7A audit repair passed for backend/storage/context gaps recorded in `docs/testing/NS-410_ACCEPTANCE.md`; repaired Slice 8 passed for Write embedded progression blocks and scene progression panel; Slice 9 passed for automated Codex baseline/history/effective-at-scene UI scope; the post-Slice 9 continuous Write editor refactor plus inline Codex progression/autosave repair has automated command checks passing; next is Slice 10 remaining JSON authority migration after this refactor is committed.
