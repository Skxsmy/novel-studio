...
- [-] `M5` Workshop, Proposal, and Review. `M5.1-M5.5` plus the follow-up Workshop context, General Chat, attachments, streaming/reasoning, session lifecycle, export, Chat/Agent split, limited Agent Codex create/update tools, shared sidebar/floating menus, prompt isolation, provider-context filtering, and pending-draft follow-up repairs are command/function verified. The current function/UI/backend map is `docs/design/ui-redesign/M5_WORKSHOP_CURRENT_FUNCTION_AND_UI_MAP.md`. The current verified defect source is `docs/testing/M5_WORKSHOP_VERIFIED_DEFECT_AUDIT_2026-07-09.md`; the older Workshop functional and prompt/call-chain audit records were deleted because they mixed pre-repair and post-repair states. Next slice: `M5.6` verified Agent/tool foundation, with the original Tool Plan/Grant requirements preserved and classified in `docs/tasks/M5.md`; only the old monolithic implementation order is replaced. User visual acceptance remains pending, and agent-owned screenshot acceptance is prohibited.
