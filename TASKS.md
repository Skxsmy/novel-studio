...
- Done in current M5 code slice: Workshop persists sessions/messages/context baskets, reloads durable messages, previews context through the existing Context Builder, selects library-global model settings plus per-call provider model overrides, and runs single-role calls without authority mutation.
