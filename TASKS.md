...
- [-] `M5` Workshop, Proposal, and Review. `M5.1-M5.5` plus post-M5.5 audit repair are command/function verified on 2026-07-01. The 2026-07-02 Workshop context-selector repair, Codex detail `Send to AI` audit, and Workshop General Chat repair are command/function verified. Next slice: `M5.6` Tool Plan, Grant, And Command Adapter. User visual acceptance remains pending, and agent-owned screenshot acceptance is prohibited.
...
- Done in current M5 code slice: Workshop persists sessions/messages/context baskets, reloads durable messages, assembles context through the existing Context Builder, selects library-global model settings plus per-call provider model overrides, and runs single-role calls without authority mutation. The visible UI now edits selected context through a compact composer menu instead of a permanent right-side Context Basket panel, while the internal storage object remains `WorkshopContextBasket`.
...
- Done in current M5 code slice: Workshop General Chat is the default call mode, can send a user-edited system prompt into Context Builder, displays the author's message immediately after Send, appends the assistant reply or model-error message when the call resolves, and never exposes Create Proposal for General Chat replies. The system prompt editor opens from a compact footer button into a larger popover instead of living as a cramped footer textarea. Storage/API also reject Proposal creation from General Chat messages.
