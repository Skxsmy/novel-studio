...
- Done in current NS-410 Slice 8: Write can insert embedded story-change blocks, create/update linked field Progression records, preview same-scene before/after effective Codex values, collapse block UI state without persisting it to authority, list current-scene progression blocks in Scene Brief, and delete block plus linked Progression together or show blocker reasons without mutating either record.
