...
- [-] `M5` Workshop, Proposal, and Review. `M5.1-M5.5` plus the follow-up Workshop context, General Chat, attachments, streaming/reasoning, session lifecycle, export, Chat/Agent split, limited Agent Codex create/update tools, shared sidebar/floating menus, prompt isolation, provider-context filtering, pending-draft follow-up, and `M5.6A` Tool Execution Containment repairs are command/function verified. The current function/UI/backend map is `docs/design/ui-redesign/M5_WORKSHOP_CURRENT_FUNCTION_AND_UI_MAP.md`. The current verified defect source is `docs/testing/M5_WORKSHOP_VERIFIED_DEFECT_AUDIT_2026-07-09.md`; the older Workshop functional and prompt/call-chain audit records were deleted because they mixed pre-repair and post-repair states. Next slice: `M5.6B` Atomic Codex adapters, with the original Tool Plan/Grant requirements preserved and classified in `docs/tasks/M5.md`; only the old monolithic implementation order is replaced. User visual acceptance remains pending, and agent-owned screenshot acceptance is prohibited.
- [ ] `M5.6B` must begin with the re-audit's per-series file-transaction coordinator and concurrent recovery tests, then implement the limited atomic Codex adapter, draft baselines, approval-payload identity, and Progression binding. This is part of the active M5 line, not a separate milestone.
...
- Current source files:
  ...
  - `docs/testing/M5_WORKSHOP_VERIFIED_DEFECT_AUDIT_2026-07-09.md`
...
- Done in current M5 code slice: Workshop conversations are created as fixed `chat` or `agent` sessions. General Chat remains discussion-only and cannot create Proposals or write actions. General Chat and Agent prompts are defined in the Workshop-specific prompt module and are isolated from global role/template records. Agent sessions use a server-side structured-step protocol; fake `Tool Call` text is suppressed and cannot execute, while valid structured Agent steps create server-owned `role: tool` JSON messages. The limited author-confirmed `codex.create_entry` tool can create new Codex entries from Agent tool messages only, matches Details to existing reusable detail type IDs or exact normalized names, prepares missing detail type creation for confirmation, creates those detail types after confirmation, and writes entry Details by stable IDs. The limited author-confirmed `codex.update_entry` tool can update one existing Codex entry's fields/research/details and create/update/delete unified Codex Progression records through the validated repository commands. Explicit author authorization in an Agent conversation is treated as sufficient source basis for drafting/calling these tools. Relation/category/knowledge writes, durable multi-step continuation, and general Codex command adapters remain future work.
