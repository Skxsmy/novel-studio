...
- Done in current NS-410 Write editor refactor follow-up: Write now uses a continuous Tiptap-backed manuscript editor instead of the visible repeated block-card/textarea UI. `SceneBlockDocument` remains the saved authority, paragraph add/delete and Focus remain available, the floating format panel/destructive text-type selector was removed, legacy heading/scene-break data still round-trips, Codex progression creation/edit/delete/collapse now happens only through a resizable inline manuscript component that defaults collapsed and expands explicitly for editing, Scene Brief no longer duplicates Codex progression state, Write/Codex progression/Codex entry editing autosaves without normal Save buttons, and automated command checks pass. User visual acceptance remains separate.
