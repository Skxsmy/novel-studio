...
- Done in current NS-410 Write editor refactor follow-up: Write now uses a continuous Tiptap-backed manuscript editor instead of the visible repeated block-card/textarea UI. `SceneBlockDocument` remains the saved authority, paragraph add/delete and Focus remain available, the floating format panel/destructive text-type selector was removed, legacy heading/scene-break data still round-trips, story-change creation/edit/delete/collapse moved to compact manuscript anchors plus the Scene Brief panel, and automated command checks pass. User visual acceptance remains separate.
