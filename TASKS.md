...
- Open follow-up after the current bugfix commit: replace the ad hoc contentEditable approach with an editor slice that evaluates a document/selection/transaction model, decoration-based Codex marks, IME-safe input handling, paste cleanup, undo/redo, paragraph indentation rules, scroll-aware anchored popovers, and writer-focused status/word-count behavior. Do not keep adding unrelated one-off key handlers indefinitely.
