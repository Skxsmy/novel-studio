...
- Current planning decision: the prior "do not start NS-410" warning is superseded by explicit user direction on 2026-06-26. M2/M3 foundations can be modified for the JSON authority, block document, and unified Progression change. Slices 1-9, the Slice 7A/Slice 8 review repairs, and the user-requested project lifecycle repair for Trash/Restore/permanent delete are implemented on the NS-410 branch; continue with Slice 10 unless the user redirects.
...
- Write now has ordinary blocks, embedded progression blocks, and a current-scene progression panel for field Progression records. Repaired Slice 8 uses scene-level atomic create/delete commands for embedded progression blocks, saves dirty scene drafts before deleting a story-change block, and previews the first story-change block from the previous narrative scene's effective state.
