...
- Latest committed baseline before this repair: `NS-410 feat(write): add progression blocks` on this branch.
...
- Current planning decision: the prior "do not start NS-410" warning is superseded by explicit user direction on 2026-06-26. M2/M3 foundations can be modified for the JSON authority, block document, and unified Progression change. Slices 1-8, the Slice 7A/Slice 8 review repairs, and the user-requested project lifecycle repair for Trash/Restore/permanent delete are implemented on the NS-410 branch; continue with Slice 9 unless the user redirects.
...
- Codex still needs baseline/history/effective-at-scene views for progressed fields.

Implementation order is now the controlled vertical slice plan in `docs/tasks/NS-410.md`; acceptance evidence and slice exits are in `docs/testing/NS-410_ACCEPTANCE.md`. Slice 1 passed for its scope on 2026-06-26; Slices 2, 3, 4, 5, 6, 7, 7A audit repair, and repaired Slice 8 passed for their scopes on 2026-06-29. Continue with Slice 9 Codex baseline/history/effective-at-scene UI next; do not skip to the remaining JSON authority migration slice.
...
Continue with Slice 9: Codex baseline/history/effective-at-scene UI. Each slice must leave the app buildable and record its exit checks before the next slice starts. Do not skip to provider expansion, project deletion, broad remaining JSON migration, or fake Proposal integration in Slice 9. Whole-project YAML/Markdown authority migration is still required later in NS-410 Slice 10, not silently dropped, and routine handoff must stay in the existing files.
