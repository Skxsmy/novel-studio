...
- Latest committed baseline before this Slice 9 pass: `51c8011 NS-410 fix(write): close progression and project deletion gaps` on this branch.
...
- Current planning decision: the prior "do not start NS-410" warning is superseded by explicit user direction on 2026-06-26. M2/M3 foundations can be modified for the JSON authority, block document, and unified Progression change. Slices 1-9, the Slice 7A/Slice 8 review repairs, the user-requested project lifecycle repair for Trash/Restore/permanent delete, and the post-Slice 9 continuous Write editor refactor plus inline Codex progression/autosave repair are implemented and command-verified on the NS-410 branch; continue with Slice 10 only after this refactor is committed unless the user redirects.
...
- Do not reintroduce the visible repeated block-card/textarea Write manuscript UI. The current product decision is a continuous manuscript editor with paragraph add/delete and Codex progression actions preserved through compact editor commands, resizable inline Codex progression components, and a Scene Brief current-scene progression list/read-only overview.
...
- Write now has a continuous Tiptap-backed manuscript editor over `SceneBlockDocument`, resizable inline Codex progression components, and a Scene Brief current-scene progression list/read-only overview for field Progression records. The floating formatting panel/current-text type selector is removed; scene-break is not exposed as a destructive conversion for active prose. Repaired Slice 8 uses scene-level atomic create/delete commands for Codex progressions, saves dirty scene drafts before deleting a Codex progression, and previews the first Codex progression from the previous narrative scene's effective state.
- Codex now has a command-verified Progressions tab that separates saved initial state from selected-scene effective field state and field-grouped Progression history without rendering Progression IDs/revisions in the author workflow. User visual acceptance is not claimed.

Implementation order is now the controlled vertical slice plan in `docs/tasks/NS-410.md`; acceptance evidence and slice exits are in `docs/testing/NS-410_ACCEPTANCE.md`. Slice 1 passed for its scope on 2026-06-26; Slices 2, 3, 4, 5, 6, 7, 7A audit repair, repaired Slice 8, and Slice 9 automated UI scope passed on 2026-06-29. The post-Slice 9 Write editor refactor is a user-directed, command-verified follow-up before Slice 10. Continue with Slice 10 remaining JSON authority migration next only after the editor refactor is committed; do not skip to final handoff while the remaining-format inventory is still open.
...
Continue with Slice 10: remaining JSON authority migration. Each slice must leave the app buildable and record its exit checks before the next slice starts. Do not skip to provider expansion, fake Proposal integration, or final handoff while the remaining YAML/Markdown authority inventory is still open. Whole-project YAML/Markdown authority migration is still required in NS-410 Slice 10, not silently dropped, and routine handoff must stay in the existing files.
