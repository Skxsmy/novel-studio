...
- Workshop and Review are implemented only through M5.5 plus post-M5.5 audit repair. Review has real Proposal inbox/detail/decision flows including stale marking, full patch review, revision-bound batch accept, and batch result display. Workshop has durable sessions/messages/context baskets/context preview/single-role calls plus transactional Workshop-to-Proposal deep links, source-message status sync, unavailable-state recovery, and current UI context-source additions. Tool Plans/Grants, Council, and final user visual acceptance are incomplete.
