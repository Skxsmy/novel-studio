...
- Current Workshop audit reports: `docs/design/ui-redesign/M5_WORKSHOP_CURRENT_FUNCTION_AND_UI_MAP.md` maps the current Workshop controls and frontend-backend-storage behavior; `docs/testing/M5_WORKSHOP_VERIFIED_DEFECT_AUDIT_2026-07-09.md` was fully rechecked after M5.6A against `aea8998` plus the lifecycle follow-up. It records per-series transaction concurrency, atomic adapter/baseline/Progression gaps, execution recovery, immutable prompt provenance, approval-payload identity, semantic detail planning, durable Agent execution, capability cleanup, prompt customization, and Tool Plans/Grants as open. Disabled Import Thread is an honest deferred state. Start M5.6B with a per-series transaction coordinator before composing larger Codex transactions. `docs/tasks/M5.md` keeps the original M5.6 requirements and replaces only the monolithic order.
...
- Current planning decision: the prior "do not start NS-410" warning is superseded by explicit user direction on 2026-06-26. M2/M3 foundations can be modified for the JSON authority, block document, and unified Progression change. Slices 1-11, the Slice 7A/Slice 8 review repairs, the user-requested project lifecycle repair for Trash/Restore/permanent delete, the post-Slice 9 continuous Write editor refactor, and the post-Slice 10 Codex editor bounded-scroll/background-autosave repair are implemented and command-verified on the NS-410 branch. The local `docs/testing/NS-410_SLICE_1_11_AUDIT.md` file is retained but must not be committed unless the user later says otherwise. The local `docs/testing/M5_1_M5_4_CODE_UI_AUDIT.md` and `docs/testing/M5_5_CODE_UI_AUDIT.md` reports were used for post-M5.5 audit repair and should remain uncommitted unless the user explicitly says otherwise. Workshop General Chat is now default, discussion-only, custom-system-prompt-capable with no hidden fallback prompt, optionally streamed, reasoning-aware with visible collapsed state, manually stoppable while calls are in flight, deletable for unlinked messages, editable/resendable for successful author messages in `chat` sessions with durable forward-history truncation, session-exportable as requested-session UTF-8 Markdown with separate saved-reasoning and prompt-audit options, and barred from Proposal creation. Workshop session export keeps attachment file records but omits extracted attachment body text. Workshop sessions are fixed as `chat` or `agent` at creation. Agent sessions use a server-side structured-step protocol: raw simulated `Tool Call` text is suppressed and cannot execute, while valid structured Agent steps create server-owned `role: tool` JSON messages. Agent sessions do not expose or accept General Chat edit/resend in this slice. The limited author-confirmed `codex.create_entry` and `codex.update_entry` tools execute only from server-owned Agent tool messages; explicit author authorization is sufficient source basis; reusable Details use stable detail-type IDs; current tool messages persist execution state; storage serializes per-session claims in the single local server process; concurrent/repeated execution and archived-session execution are refused before duplicate authority writes; and running execution blocks session archive/delete. `codex.update_entry` can update one existing Codex entry's fields/research/details and create/update/delete unified Codex Progression records through existing repository commands. Workshop sessions do not bind a scene by default, and selected context, parsed message attachments, same-session chat history, provider reasoning, session naming/branching, and permanent unlinked-session deletion retain their documented behavior. M5.6A is command/function verified. The next slice is M5.6B Atomic Codex adapters for transactionality, stale baselines, and Progression binding, followed by detail planning, durable Agent runner, capability cleanup, prompt customization, and the preserved original Tool Plan/Grant target as M5.6G. Relation, category, character-knowledge, broad world/Write mutations, and durable multi-step continuation remain future work.
...
8. `docs/testing/M5_WORKSHOP_VERIFIED_DEFECT_AUDIT_2026-07-09.md`
...
Continue with M5.6 through `docs/tasks/M5.md`, `docs/testing/M5_ACCEPTANCE.md`, and `docs/testing/M5_WORKSHOP_VERIFIED_DEFECT_AUDIT_2026-07-09.md`. M5.6A is command/function verified; start M5.6B with atomic Codex adapters, stale baselines, and Progression binding, then continue detail schema planning, durable Agent runner, Workshop capability cleanup, Workshop prompt customization, and the original Tool Plan/Grant/Tool Call storage/API, approved Codex/Write tool definitions, validated command adapters, permission/result UI, and Proposal fallback before Council or broad UI polish. Do not reintroduce the visible right-side Workshop Context Basket, do not expose Create Proposal for General Chat replies, do not treat message attachments as Reference Library SourceDocuments, do not drop same-session chat history from Workshop model calls, do not expose General Chat edit/resend in Agent sessions, do not export attachment body text in Workshop session export, do not reintroduce a Workshop-specific app project sidebar rail, and do not use screenshot-based visual acceptance. Do not regress the limited Agent tool protocol: explicit author authorization is sufficient source basis for limited Codex tool drafting, `codex.update_entry` progression create/update/delete support exists, and current tool execution claims/repeated-call refusal remain active, but none of these are full M5.6 Tool Plans/Grants. NS-410 closed the runtime YAML/Markdown authority inventory; Markdown/Word remain boundary formats, and remaining YAML mentions are historical docs or third-party optional peer metadata, not runtime authority paths. Routine handoff must stay in the existing files.
