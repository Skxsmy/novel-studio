...
- Current acceptance state: Project Recovery is accepted for the current stage by user decision on 2026-06-30. NS-410 automated/command acceptance is closed through post-Slice 11 audit repair. M5 formally started on 2026-07-01; M5.1-M5.5 plus post-M5.5 audit repair are command/function verified. The 2026-07-02 Workshop context-selector repair, Codex detail `Send to AI` audit, and Workshop General Chat repair are also command/function verified, while user visual acceptance is still pending.
...
- Workshop and Review are implemented only through M5.5 plus post-M5.5 audit repair and follow-up UI/model-selection/context-selector/General Chat repair. Review has real Proposal inbox/detail/decision flows including stale marking, full patch review, revision-bound batch accept, batch result display, and read-only non-pending Proposal details. Workshop has durable sessions/messages/context baskets/context assembly, a compact composer context selector, full-novel/full-outline/act/chapter/multi-scene context scope support, auto-linked Codex materialization for policy-allowed entries, library-global model setting selection, per-call provider model overrides, single-role calls, default General Chat with custom system prompt, immediate author-message display after Send, model-error chat messages when calls fail, transactional Workshop-to-Proposal deep links for non-General replies, source-message status sync, and unavailable-state recovery. The visible right-side Context Basket and separate Preview Context controls should not be reintroduced. General Chat replies must not expose Create Proposal and are rejected by storage/API if proposal creation is attempted. Tool Plans/Grants, Council, and final user visual acceptance are incomplete.

Current boundaries:

- M5.1-M5.5 and post-M5.5 audit repair are command/function verified as of 2026-07-01. The 2026-07-02 Workshop context-selector and General Chat repairs are command/function verified. M5 Workshop/Proposal/Review is not complete or visually accepted; next slice is M5.6.
