...
- Latest committed baseline before this UI repair pass: `4b02c32 NS-410 fix(write): embed autosaving Codex progressions` on this branch.
...
- Do not reintroduce the visible repeated block-card/textarea Write manuscript UI. The current product decision is a continuous manuscript editor with paragraph add/delete and Codex progression actions preserved through compact editor commands and resizable inline Codex progression components. Do not reintroduce the duplicated Scene Brief Codex progression list/read-only overview.
...
- Write now has a continuous Tiptap-backed manuscript editor over `SceneBlockDocument` and resizable inline Codex progression components for field Progression records. The duplicated Scene Brief progression panel and constant Write before/after preview are removed; effective-at-scene review belongs in the Codex Progressions tab. The floating formatting panel/current-text type selector is removed; scene-break is not exposed as a destructive conversion for active prose. Repaired Slice 8 uses scene-level atomic create/delete commands for Codex progressions and saves dirty scene drafts before deleting a Codex progression.
