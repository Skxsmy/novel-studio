...
- Do not reintroduce the visible repeated block-card/textarea Write manuscript UI. The current product decision is a continuous manuscript editor with paragraph add/delete and story-change actions preserved through compact editor commands and the Scene Brief story-change panel.
...
- Write now has a continuous Tiptap-backed manuscript editor over `SceneBlockDocument`, compact manuscript story-change anchors, and a Scene Brief story-change panel for field Progression records. The floating formatting panel/current-text type selector is removed; scene-break is not exposed as a destructive conversion for active prose. Repaired Slice 8 uses scene-level atomic create/delete commands for story changes, saves dirty scene drafts before deleting a story change, and previews the first story change from the previous narrative scene's effective state.
