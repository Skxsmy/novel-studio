...
- Workshop and Review are implemented only through M5.5 plus post-M5.5 audit repair and follow-up UI/model-selection/context-selector/General Chat repair. Review has real Proposal inbox/detail/decision flows including stale marking, full patch review, revision-bound batch accept, batch result display, and read-only non-pending Proposal details. Workshop has durable sessions/messages/context baskets/context assembly, a compact composer context selector, full-novel/full-outline/act/chapter/multi-scene context scope support, auto-linked Codex materialization for policy-allowed entries, library-global model setting selection, per-call provider model overrides, single-role calls, default General Chat with a fully visible custom system prompt, optional streaming, separate collapsible reasoning display, immediate author-message display after Send, model-error chat messages when calls fail, deletion for unlinked chat messages, transactional Workshop-to-Proposal deep links for non-General replies, source-message status sync, and unavailable-state recovery. Workshop does not bind a scene by default; context must come from the user's selected context menu choices. The visible right-side Context Basket, separate Preview Context controls, and scattered footer/header settings controls should not be reintroduced. General Chat replies must not expose Create Proposal and are rejected by storage/API if proposal creation is attempted. Tool Plans/Grants, Council, and final user visual acceptance are incomplete.

Current boundaries:

- M5.1-M5.5 and post-M5.5 audit repair are command/function verified as of 2026-07-01. The 2026-07-02 Workshop context-selector repair and 2026-07-03 Workshop General Chat/settings repair are command/function verified. M5 Workshop/Proposal/Review is not complete or visually accepted; next slice is M5.6.
...
- Review and Workshop are not complete beyond M5.5 plus the 2026-07-02 context-selector repair. M5.6 must add Tool Plans/Grants/adapters; M5.7 must add Council, state sweep, and final user visual acceptance.
