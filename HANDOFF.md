...
- Latest committed baseline before this default-collapse UI repair pass: `36b7924 NS-410 fix(write): remove duplicated progression UI` on this branch.
...
- Write now has a continuous Tiptap-backed manuscript editor over `SceneBlockDocument` and resizable inline Codex progression components for field Progression records. Those components default collapsed after creation and on scene load, and expand explicitly for editing target entry/field/operation/summary/body. The duplicated Scene Brief progression panel and constant Write before/after preview are removed; effective-at-scene review belongs in the Codex Progressions tab. The floating formatting panel/current-text type selector is removed; scene-break is not exposed as a destructive conversion for active prose. Repaired Slice 8 uses scene-level atomic create/delete commands for Codex progressions and saves dirty scene drafts before deleting a Codex progression.
