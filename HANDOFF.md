...
- Current acceptance state: Project Recovery is accepted for the current stage by user decision on 2026-06-30. NS-410 automated/command acceptance is closed through post-Slice 11 audit repair. M5 formally started on 2026-07-01; M5.1-M5.5 plus post-M5.5 audit repair are command/function verified, while user visual acceptance is still pending.
...
- Workshop and Review are implemented only through M5.5 plus post-M5.5 audit repair and follow-up UI/model-selection repair. Review has real Proposal inbox/detail/decision flows including stale marking, full patch review, revision-bound batch accept, batch result display, and read-only non-pending Proposal details. Workshop has durable sessions/messages/context baskets/context preview, library-global model setting selection, per-call provider model overrides, single-role calls, transactional Workshop-to-Proposal deep links, source-message status sync, unavailable-state recovery, and current UI context-source additions. Tool Plans/Grants, Council, and final user visual acceptance are incomplete.

Current boundaries:

- M5.1-M5.5 and post-M5.5 audit repair are command/function verified as of 2026-07-01. M5 Workshop/Proposal/Review is not complete or visually accepted; next slice is M5.6.
...
- Review and Workshop are not complete beyond M5.5. M5.6 must add Tool Plans/Grants/adapters; M5.7 must add Council, state sweep, and final user visual acceptance.
