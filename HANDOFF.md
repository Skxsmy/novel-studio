...
- Future Proposal storage remains a `.json` design in architecture docs, not a completed runtime implementation.
...
- M5.0 Current Feature Protection Lock passed on 2026-07-01. M5 Workshop/Proposal/Review is not implemented or accepted yet.
...
Continue with M5.1 Proposal v2 Contract And State Machine through `docs/tasks/M5.md` and `docs/testing/M5_ACCEPTANCE.md`. Do not skip to provider expansion, fake Proposal integration, Proposal storage/API, or Workshop/Review UI implementation. M5.1 must upgrade the existing Proposal contract without creating a parallel model. NS-410 closed the runtime YAML/Markdown authority inventory; Markdown/Word remain boundary formats, and remaining YAML mentions are historical docs or third-party optional peer metadata, not runtime authority paths. Routine handoff must stay in the existing files.
