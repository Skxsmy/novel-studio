...
- Current Workshop audit reports: `docs/design/ui-redesign/M5_WORKSHOP_CURRENT_FUNCTION_AND_UI_MAP.md` maps the current Workshop controls and frontend-backend-storage behavior for future UI redesign; `docs/testing/M5_WORKSHOP_VERIFIED_DEFECT_AUDIT_2026-07-09.md` is the current-source verified defect audit after deleting the older mixed-state functional and prompt/call-chain audit files and is updated through M5.6A. The verified audit records what remains open: durable Tool Plans/Grants, atomic adapters, stale baselines, Progression binding, semantic detail planning, prompt customization, and full multi-step Agent execution. `docs/tasks/M5.md` classifies each original M5.6 item as kept, partially started, or replaced; the replaced part is only the old monolithic implementation order.
