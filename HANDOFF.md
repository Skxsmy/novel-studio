...
- Proposal v2 JSON storage and Review/proposal APIs are implemented through M5.2. Workshop-generated Proposal cards and exact source-message deep links remain M5.5 work, not completed behavior.
...
- Workshop and Review are implemented only through M5.4. Review has real Proposal inbox/detail/decision flows, and Workshop has durable sessions/messages/context baskets/context preview/single-role calls. Workshop-to-Proposal deep links, source-message status sync, Tool Plans/Grants, Council, and final user visual acceptance are incomplete.

Current boundaries:

- M5.1-M5.4 are command/function verified as of 2026-07-01. M5 Workshop/Proposal/Review is not complete or visually accepted; next slice is M5.5.
...
- Review and Workshop are not complete beyond M5.4. M5.5 must add Workshop message to Proposal deep links and status sync; M5.6 must add Tool Plans/Grants/adapters; M5.7 must add Council, state sweep, and final user visual acceptance.
...
Continue with M5.5 Workshop Message To Proposal Deep Link through `docs/tasks/M5.md` and `docs/testing/M5_ACCEPTANCE.md`. Do not skip to Tool Plans, Grants, Council, or broad UI polish. M5.5 must create durable Proposal records from Workshop messages, provide exact Review Proposal Detail links, handle missing/archived source states, and sync Review decisions back to the source Workshop message. NS-410 closed the runtime YAML/Markdown authority inventory; Markdown/Word remain boundary formats, and remaining YAML mentions are historical docs or third-party optional peer metadata, not runtime authority paths. Routine handoff must stay in the existing files.
