...
- Workshop Agent authorization and `codex.update_entry` progression repair validation on 2026-07-08:
  ...
  - `git diff --check`: passed with Windows line-ending warnings only.
  - Regressions verify that explicit author authorization triggers a repair pass instead of persisting an evidence-refusal Agent reply, and that confirmed `codex.update_entry` tool messages can create, update, and delete unified Codex Progression records.
...
- Workshop workbench-shell/conversation/context-panel redesign validation on 2026-07-08:
  ...
  - Scope: Workshop-specific full-height app shell, narrow rail, topbar New/More session actions, compact thread dock, active conversation header, broad role-accented message flow, message action menus, compact composer, and tabbed Story scope / Structure / Codex / Files context selector. Existing storage/server/API behavior was not changed.
